                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped table-bordered table-hover">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td>{{$products->currentpage()==1?$i+1:$i+($products->perpage()*($products->currentpage() - 1))+1}}</td>
                                                <td>{{$product->name}}</td>
                                                <td>
                                                ...
                                                <td>
                                                    ...
                                                    {{priceFormat($product->productVariationAttributeItems()->avg('purchase_price'))}}
                                                    @else
                                                ...
                                                <td>
                                                    @if($product->variation_status)
                                                    {{priceFormat($product->productVariationAttributeItems()->avg('wholesale_price'))}}
                                                    @else
                                                    {{priceFormat($product->purchase_price)}}
                                                    @endif
                                                </td>
                                                <td>
                                                    @if($product->variation_status)
                                                    {{priceFormat($product->productVariationAttributeItems()->avg('final_price'))}}
                                                    @else
                                                    {{priceFormat($product->final_price)}}
                                                    @endif
                                                </td>
                                                <td>{{$product->quantity}}</td>
                                                <td>{{$product->salesAll()->count()}}</td>
                                                <td>{{priceFormat($product->salesAll()->whereHas('order',function($q){$q->where('payment_status','paid');})->sum('final_price'))}}</td>
                                                <td>{{priceFormat($product->salesAll()->whereHas('order',function($q){$q->where('payment_status','unpaid');})->sum('final_price'))}}</td>
                                            </tr>
                                            @endforeach
