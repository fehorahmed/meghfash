...
@if($parentOrder =$order->parentOrder)
    ...
    <table class="table table-bordered">
        ...
        <tr>
            <td>{{$i+1}}</td>
            <td>
                <img src="{{asset($rItem->image())}}" style="max-height: 40px;max-width: 100%;">
            </td>
            <td>
            ...
            <td style="padding:5px;">
                <input type="number" value="{{$rItem->return_quantity}}" class="form-control form-control-sm itemUpdate" data-url="{{route('admin.ordersReturnAction',['item-quantity',$order->id])}}" data-id="{{$rItem->id}}" >
            </td>
