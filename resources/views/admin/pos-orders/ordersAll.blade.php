...
<title>{{websiteTitle('POS Orders List')}}</title>
@endsection @push('css')
...
<div class="content-header row">
    ...
    <div class="content-header-right col-md-6 col-12 mb-md-0 mb-2">
        <div class="btn-group float-md-right" role="group" aria-label="Button group with nested dropdown">
            <a class="btn btn-outline-primary" href="{{route('admin.posOrdersAction','create')}}">New Order</a>
    ...
    <div class="card-content">
        <div id="accordion">
                ...
                <div id="collapseTwo" class="collapse" aria-labelledby="headingTwo" data-parent="#accordion" style="border: 1px solid #00b5b8; border-top: 0;">
                    <div class="card-body">
                        <form action="{{route('admin.posOrders')}}">
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-1">
                                    <div class="input-group">
                                        <input type="text" name="search" value="{{request()->search?:''}}" placeholder="Invoice no, Customer Mobile, email" class="form-control {{$errors->has('search')?'error':''}}" />
...
<div class="card">
    ...
    <div class="card-content">
        <div class="card-body">
            ...
            <!--                    @isset(json_decode(Auth::user()->permission->permission, true)['posUrders']['manage'])-->
            <!--                    <option value="1">Pending</option>-->
            <!--                    <option value="4">Delivered</option>-->
            <!--                    <option value="5">Cancelled</option>-->
            <!--                    @endisset-->
                                <!--@isset(json_decode(Auth::user()->permission->permission, true)['posUrders']['delete'])-->
            ...
            <!--        <div class="col-md-8">-->
            <!--            <ul class="statuslist">-->
            <!--                <li><a href="{{route('admin.posOrders')}}">All ({{$totals->total}})</a></li>-->
            <!--                <li><a href="{{route('admin.posOrders',['status'=>'pending'])}}">Pending ({{$totals->pending}})</a></li>-->
            <!--                <li><a href="{{route('admin.posOrders',['status'=>'delivered'])}}">Delivered ({{$totals->delivered}})</a></li>-->
            <!--                <li><a href="{{route('admin.posOrders',['status'=>'cancelled'])}}">Cancelled ({{$totals->cancelled}})</a></li>-->
            <!--            </ul>-->
            <!--        </div>-->
            <!--    </div>-->
                ...
                <table class="table table-striped table-bordered table-hover" >
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><a href="{{route('admin.posOrdersAction',['invoice',$order->id])}}" target="_blank">{{$order->invoice}}</a></td>
                                <td>{{$order->name}} - {{$order->mobile}}</td>
                                <td>
                                    {{App\Models\General::first()->currency}}
                                    {{number_format($order->grand_total)}}
                                    @if($order->payment_status=='parftial')
                                    <span class="badge badge-success" style="background:#ff9800;">{{ucfirst('Paid')}}</span>
                                    @elseif($order->payment_status=='paid' || $order->payment_status=='partial')
                                    <span class="badge badge-success" style="background:#673ab7;">{{ucfirst('Paid')}}</span>
                                    @else
                                ...
                                <td>
                                    ...
                                    <!--<a href="{{route('admin.posOrdersAction',['exchange',$order->id])}}" class="btn btn-sm btn-info">Exchange</a>-->
                                    @else
                                    <a href="{{route('admin.posOrdersAction',['edit',$order->id])}}" class="btn btn-sm btn-success">Edit</a>
                                    @endif
                                    @endisset
                                </td>
                            </tr>
