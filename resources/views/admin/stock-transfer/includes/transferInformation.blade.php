<div class="row">
    <div class="col-md-3">
        <div class="form-group">
            ...
            <input type="text" readonly value="{{$invoice->formBranch?$invoice->formBranch->name:''}}" class="form-control">
            @else
            <select class="form-control selectWarehouse2"  data-url="{{route('admin.stockTransferAction',['formwarehouse-select',$invoice->id])}}">
                <option value="">Select Store/Branch</option>
                @foreach($warehouses as $warehouse)
                <option value="{{$warehouse->id}}" {{$warehouse->id==$invoice->user_id?'selected':''}}>{{$warehouse->name}}</option>
                @endforeach
            </select>
            @endif
    ...
    <div class="col-md-3">
        <div class="form-group">
            ...
            @else
            <select class="form-control selectWarehouse"  data-url="{{route('admin.stockTransferAction',['towarehouse-select',$invoice->id])}}">
                <option value="">Select Store/Branch</option>
                @foreach($warehouses as $warehouse)
