...
@section('title')
<title>{{websiteTitle('Stock Transfer')}}</title>
@endsection
@push('css')
<style type="text/css">
...
<div class="content-header row">
    ...
    <div class="content-header-right col-md-6 col-12 mb-md-0 mb-2">
        <div class="btn-group float-md-right" role="group" aria-label="Button group with nested dropdown">
            ...
            <a class="btn btn-outline-primary" href="{{route('admin.stockTransferAction','create')}}">New Transfer</a>
            @endisset
            <a class="btn btn-outline-primary" href="{{route('admin.stockTransfer')}}">
    ...
    <div class="card-content">
        <div id="accordion">
            ...
            <div id="collapseTwo" class="collapse" aria-labelledby="headingTwo" data-parent="#accordion" style="border: 1px solid #00b5b8; border-top: 0;">
                <div class="card-body">
                    <form action="{{route('admin.stockTransfer')}}">
                        <div class="row">
        ...
        <div class="card-body">
            <form action="{{route('admin.stockTransfer')}}">
                <div class="row">
                    ...
                    <div class="col-md-8">
                        <ul class="statuslist">
                            <li><a href="{{route('admin.stockTransfer')}}">All ({{$totals->total}})</a></li>
                            <li><a href="{{route('admin.stockTransfer',['status'=>'pending'])}}">Pending ({{$totals->pending}})</a></li>
                            <li><a href="{{route('admin.stockTransfer',['status'=>'delivered'])}}">Delivered ({{$totals->delivered}})</a></li>
                            <li><a href="{{route('admin.stockTransfer',['status'=>'cancelled'])}}">Cancelled ({{$totals->cancelled}})</a></li>
                        </ul>
                    </div>
                </div>
                        ...
                        <tbody>
                            @foreach($invoices as $i=>$invoice)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{route('admin.stockTransferAction',['invoice',$invoice->id])}}">{{$invoice->invoice}}</a>
                                        <a href="{{route('admin.stockTransferAction',['edit',$invoice->id])}}" class="badge badge-info"><i class="fa fa-edit"></i></a>
                                    </td>
                                    <td>
                                        @if($invoice->formBranch)
                                        {{$invoice->formBranch->name}}
                                        @endif
                                    </td>
