...
@section('title')
<title>{{websiteTitle('Stock Minus')}}</title>
@endsection
@push('css')
<style type="text/css">
...
<div class="content-header row">
    ...
    <div class="content-header-right col-md-6 col-12 mb-md-0 mb-2">
        <div class="btn-group float-md-right" role="group" aria-label="Button group with nested dropdown">
            ...
            <a class="btn btn-outline-primary" href="{{route('admin.stockMinusAction','create')}}">New Minus</a>
            @endisset
            <a class="btn btn-outline-primary" href="{{route('admin.stockMinus')}}">
    ...
    <div class="card-content">
        <div id="accordion">
            ...
            <div id="collapseTwo" class="collapse" aria-labelledby="headingTwo" data-parent="#accordion" style="border: 1px solid #00b5b8; border-top: 0;">
                <div class="card-body">
                    <form action="{{route('admin.stockMinus')}}">
                        <div class="row">
        ...
        <div class="card-body">
            <form action="{{route('admin.stockMinus')}}">
                <div class="row">
                    ...
                    <div class="col-md-8">
                        <ul class="statuslist">
                            <li><a href="{{route('admin.stockMinus')}}">All ({{$totals->total}})</a></li>
                            <li><a href="{{route('admin.stockMinus',['status'=>'pending'])}}">Pending ({{$totals->pending}})</a></li>
                            <li><a href="{{route('admin.stockMinus',['status'=>'delivered'])}}">Delivered ({{$totals->delivered}})</a></li>
                            <li><a href="{{route('admin.stockMinus',['status'=>'cancelled'])}}">Cancelled ({{$totals->cancelled}})</a></li>
                        </ul>
                    </div>
                </div>
                        ...
                        <tbody>
                            @foreach($invoices as $i=>$invoice)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{route('admin.stockMinusAction',['invoice',$invoice->id])}}">{{$invoice->invoice}}</a>
                                        <a href="{{route('admin.stockMinusAction',['edit',$invoice->id])}}" class="badge badge-info"><i class="fa fa-edit"></i></a>
                                    </td>
                                    <td>
                                        @if($invoice->branch)
                                        {{$invoice->branch->name}}
