...
<ul>
	@foreach($searchProducts as $searchProduct)
    	@if($searchProduct->variation_status)
        	...
        	<li data-url="{{route('admin.stockMinusAction',['add-product',$invoice->id])}}" data-id="{{$searchProduct->id}}" data-variant="{{$item->id}}">
        	    {!!$item->variationItemValues()!!} -	{{$searchProduct->name}} {{$item->barcode?'| '.$item->barcode:''}}
        	</li>
        	@endforeach
    	@else
    	<li data-url="{{route('admin.stockMinusAction',['add-product',$invoice->id])}}" data-id="{{$searchProduct->id}}" data-variant="">
