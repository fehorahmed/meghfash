...
<div class="row">
	<div class="form-group col-md-4">
        ...
        <a href="{{route('admin.productsAction',['barcode',$product->id])}}" target="_blank" style="color: #ff864a;font-size: 12px;font-weight: bold;">Print  <i class="fa fa-barcode" style="color: black;"></i> </a>
        @endif
		</label>
...
<div class="row">
	...
	<div class="form-group col-md-4">
		...
		class="form-control productDataAjaxUpdate form-control-sm purchase_price_00 {{$errors->has('purchase_price')?'error':''}}"
	...
	<div class="form-group col-md-4">
		...
		value="{{$product->wholesale_price?$product->wholesale_price:old('wholesale_price')}}" 
		data-url="{{route('admin.productsUpdateAjax',['wholesale_price',$product->id])}}" 
		class="form-control productDataAjaxUpdate form-control-sm wholesale_price_00 {{$errors->has('wholesale_price')?'error':''}}"
		placeholder="Wholesale Price">
		@if ($errors->has('wholesale_price'))
        <p style="color: red; margin: 0; font-size: 10px;">{{ $errors->first('wholesale_price') }}</p>
        @endif
	</div>
	{{--
	<div class="form-group col-md-12">
	    <div class="table-responsive">
	        <table class="table table-bordered">
	            <tr>
	                <th>Warranty Note</th>
	                <th>Warranty Charge</th>
	            </tr>
	            <tr>
	                <td>
	                    <input type="text" name="warranty_note" 
                		value="{{$product->warranty_note?$product->warranty_note:old('warranty_note')}}" 
                		data-url="{{route('admin.productsUpdateAjax',['warranty_note',$product->id])}}" 
                		class="form-control productDataAjaxUpdate form-control-sm {{$errors->has('warranty_note')?'error':''}}" 
                
                		placeholder="Enter Warranty">
	                </td>
	                <td>
	                    <input type="number" name="warranty_charge" 
                		value="{{$product->warranty_charge?$product->warranty_charge:old('warranty_charge')}}" 
                		data-url="{{route('admin.productsUpdateAjax',['warranty_charge',$product->id])}}" 
                		class="form-control productDataAjaxUpdate form-control-sm {{$errors->has('warranty_charge')?'error':''}}" 
                
                		placeholder="Warranty Charge">
	                </td>
	            </tr>
	            <tr>
	                <td>
	                    <input type="text" name="warranty_note2" 
                		value="{{$product->warranty_note2?$product->warranty_note2:old('warranty_note2')}}" 
                		data-url="{{route('admin.productsUpdateAjax',['warranty_note2',$product->id])}}" 
                		class="form-control productDataAjaxUpdate form-control-sm {{$errors->has('warranty_note2')?'error':''}}" 
                
                		placeholder="Enter Warranty">
	                </td>
	                <td>
	                    <input type="number" name="warranty_charge2" 
                		value="{{$product->warranty_charge2?$product->warranty_charge2:old('warranty_charge2')}}" 
                		data-url="{{route('admin.productsUpdateAjax',['warranty_charge2',$product->id])}}" 
                		class="form-control productDataAjaxUpdate form-control-sm {{$errors->has('warranty_charge2')?'error':''}}" 
