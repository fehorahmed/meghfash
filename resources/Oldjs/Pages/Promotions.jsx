...
export default function Promotions({ 
    ...
    promotions,
        ...
        const [filters, setFilters] = useState({
          ...
          const [filteredProductData, setFilteredProductData] = useState(promotions.products);

          const [summery, setSummery] = useState({
            total: promotions.products.total,
            from: promotions.products.from,
            to: promotions.products.to
          })
    ...
    <>
     <MainLayout
            auth={auth}
                ...
                carts={carts}
            >
                {/* <Head title={category.seo_title?category.seo_title:`${promotions.name} - ${general.web_title}`} >
                    <meta name="title" property="og:title" content={category.seo_title?category.seo_title:`${promotions.name} - ${general.web_title}`} />
                    <meta name="description" property="og:description" content={promotions.seo_description?category.seo_description:general.meta_description} />
                    <meta name="keyword" property="og:keyword" content={category.seo_keyword?category.seo_keyword:general.meta_keyword}/>
                    <meta name="image" property="og:image" content={promotions.meta_image?promotions.meta_image:general.logo_url} />
                    <meta name="url" property="og:url" content={route('productCategory', promotions.slug ? promotions.slug : 'no-title')} />
                    <link rel="canonical" href={route('productCategory', promotions.slug ? promotions.slug : 'no-title')} />
                </Head> */}
                      ...
                      <div className="row row-cols-1">
                          <div className="col">
                              <div className="breadcrumb__content text-center">
                                  <h1 className="breadcrumb__content--title text-white mb-25">{promotions.name}</h1>
                                  <ul className="breadcrumb__content--menu d-flex justify-content-center">
                                      <li className="breadcrumb__content--menu__items"><Link className="text-white" href={route('index')}>Home </Link></li>
                                     
                                      <li className="breadcrumb__content--menu__items"><span className="text-white">{promotions.name}</span></li>
                                  </ul>
                              </div>
                          </div>
                      </div>
